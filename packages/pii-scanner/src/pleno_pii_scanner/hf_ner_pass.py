...
1. Env `PLENO_PII_SCANNER_HF_MODEL` (path or HF repo id like `0xhikae/ja-ner-onnx`)
2. Default `0xhikae/ja-ner-onnx` revision `v0.13.0` from HF Hub
...
_DEFAULT_HF_MODEL = "0xhikae/ja-ner-onnx"
_DEFAULT_HF_REVISION = "v0.13.0"
...
def _resolve_model_source() -> tuple[str, str | None]:
    """Return (model, revision) — local path or HF Hub repo id."""
    explicit = os.environ.get("PLENO_PII_SCANNER_HF_MODEL")
    if explicit:
        if Path(explicit).exists():
            return explicit, None
        return explicit, os.environ.get("PLENO_PII_SCANNER_HF_REVISION")
    return _DEFAULT_HF_MODEL, os.environ.get(
        "PLENO_PII_SCANNER_HF_REVISION", _DEFAULT_HF_REVISION
    )
...
def _load_pipeline():
    ...
    model_src, revision = _resolve_model_source()
