...
### Presidio context enhancer (extension point)

The local builtin engine accepts any Presidio `ContextAwareEnhancer` via
`PlenoAnonymize(context_aware_enhancer=...)`, and applies it to both `analyze`
and `redact`. The SDK ships no enhancer of its own; the remote and OpenAI
Privacy Filter engines reject the option.

For a ready-made false-positive filter, see the optional
[`pleno-presidio-extras`](../presidio-extras) package. Its `JevContextFilter`
